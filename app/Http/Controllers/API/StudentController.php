...
use App\Jobs\ProcessFaceIndexing;
...
class StudentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Viewer identity is included in the cache key so guests and
        // authenticated users never share the same cached result.
        ...
        return Cache::remember($key, 300, function () use ($request, $viewer) {
            return User::with('section')
                ...
                ->when($viewer, fn($q) =>
                    // Authenticated: public + alumni_only, plus own profile
    ...
    // ── Achievements ──────────────────────────────────────────────────────────
    // Note: visibility is enforced by the CheckProfileVisibility middleware
    // on this route — no need to repeat the check here.
