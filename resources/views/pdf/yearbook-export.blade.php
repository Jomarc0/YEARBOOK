...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .bg-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; opacity: .32; }
        ...
        .page-no { position: absolute; left: 10mm; right: 10mm; bottom: 4mm; font-family: DejaVu Sans, Arial, sans-serif; font-size: 6.5pt; letter-spacing: 1.4pt; text-transform: uppercase; color: #9b927e; }
        ...
        .student-card { border: .25mm solid rgba(216,199,162,.62); background: rgba(255,255,255,.74); page-break-inside: avoid; overflow: hidden; }
        .student-photo-cell { width: 96mm; height: 126mm; text-align: center; vertical-align: middle; background: #e8dfcc; padding: 4mm; }
        .student-photo { max-width: 88mm; max-height: 118mm; width: auto; height: auto; object-fit: contain; background: #e8dfcc; }
        .placeholder { width: 88mm; height: 118mm; line-height: 118mm; text-align: center; background: #e8dfcc; color: #071a33; font-size: 34pt; font-weight: bold; }
        .student-name { font-size: 24pt; line-height: .98; font-weight: bold; color: #071a33; text-transform: uppercase; }
...
@foreach(array_chunk($toc, 20) as $tocChunk)
<div class="page paper">
    <div class="eyebrow">Table of</div>
    <div class="title" style="font-size:34pt;color:#c89b3c;">Contents</div>
    <div class="rule"></div>
    <table style="width:100%;border-collapse:collapse;margin-top:3mm;">
        @foreach($tocChunk as $item)
            <tr>
                <td style="width:15mm;font-size:13pt;font-weight:bold;color:#071a33;padding:1.25mm 0;">{{ str_pad((string) (($loop->parent->index * 20) + $loop->iteration), 2, '0', STR_PAD_LEFT) }}</td>
                <td style="border-bottom:.2mm solid rgba(216,199,162,.55);padding:1.25mm 0;">
                    <div style="font-family:DejaVu Sans,Arial,sans-serif;font-size:6.7pt;font-weight:bold;letter-spacing:.6pt;text-transform:uppercase;color:#071a33;">{{ $item }}</div>
                </td>
            </tr>
        @endforeach
    </table>
...
<div class="page {{ $loop->odd ? 'cream' : 'paper' }}">
    <table style="width:100%;height:164mm;border-collapse:collapse;">
        <tr>
            <td style="vertical-align:middle;padding-right:12mm;">
                <div class="eyebrow">{{ $eyebrow }}</div>
                <div class="title" style="margin-top:3mm;font-size:34pt;">{{ $heading }}</div>
                <div class="rule"></div>
                <p class="body" style="font-size:11pt;width:170mm;">{{ $message }}</p>
                <div style="margin-top:16mm;border-top:.2mm solid rgba(216,199,162,.55);padding-top:6mm;">
                    <div style="font-size:17pt;color:#071a33;">{{ $sign }}</div>
                    <div class="eyebrow" style="font-size:6pt;margin-top:1.5mm;">Class of {{ $batchYear }}</div>
                </div>
            </td>
            <td style="width:70mm;vertical-align:middle;">
                @if($facultyBase64)<img src="{{ $facultyBase64 }}" style="width:70mm;height:118mm;object-fit:cover;filter:grayscale(100%);opacity:.82;" alt="">@endif
            </td>
        </tr>
    </table>
...
<div class="page paper">
    ...
    <p class="body" style="width:160mm;">A graduating community shaped by National University tradition, academic discipline, and service.</p>
    <table style="width:100%;border-collapse:separate;border-spacing:2mm;margin-top:7mm;background:#071a33;padding:2mm;">
        <tr>
    ...
    @foreach(array_slice($stats['courses'], 0, 14, true) as $course => $count)
        <div style="border-bottom:.2mm solid rgba(216,199,162,.55);padding:2mm 0;">
...
<div class="page cream">
    ...
    <table style="width:100%;border-collapse:separate;border-spacing:2mm;margin:3mm -2mm 5mm;">
        <tr>
    ...
    @foreach(['Honors Distribution' => $stats['honors'], 'Course Distribution' => $stats['courses']] as $heading => $rows)
        ...
        @foreach(array_slice($rows, 0, 10, true) as $label => $count)
            <div style="padding:1.4mm 0;border-bottom:.2mm solid rgba(216,199,162,.55);font-size:8pt;">
...
@foreach($sections as $section)
    ...
    <div class="page navy">
        ...
        <div class="navy-overlay"></div><div class="frame"></div><div class="inner-frame"></div>
        <div class="z" style="position:absolute;left:0;right:0;top:26mm;text-align:center;font-size:70pt;line-height:1;color:rgba(255,255,255,.04);font-weight:bold;">{{ strtoupper(substr($section['name'], 0, 8)) }}</div>
        <div class="z" style="text-align:center;margin-top:34mm;">
            <div class="eyebrow">Section</div>
            <div style="font-size:50pt;line-height:.9;color:#e4c36a;font-weight:bold;">{{ $section['name'] }}</div>
            ...
            <div class="rule" style="margin:8mm auto;"></div>
    ...
    <div class="page {{ $loop->odd ? 'paper' : 'cream' }}">
        <div class="eyebrow" style="font-size:6.2pt;">{{ $section['name'] }}</div><div class="title" style="font-size:25pt;">Graduate Profile</div><div class="rule" style="margin:3mm 0;"></div>
        <div class="student-card">
            <table style="width:100%;border-collapse:collapse;">
                <tr>
                    ...
                    <td style="vertical-align:top;padding:5mm 7mm;">
                        <div class="student-name" style="{{ strlen($student['name']) > 24 ? 'font-size:19pt;' : '' }}">{{ $student['name'] }}</div>
                        <div class="small" style="margin-top:1.5mm;font-size:6.4pt;">{{ $student['course'] ?: $student['student_id'] }}</div>
                        @if($student['honors'])<span class="honor">{{ $student['honors'] }}</span>@endif
                        @if($student['quote'] || $student['motto'])<div class="field" style="margin-top:3mm;border-left:.6mm solid #c89b3c;padding-left:3mm;font-size:8.5pt;line-height:1.55;font-style:italic;">“{{ $limit($student['quote'] ?: $student['motto'], 210) }}”</div>@endif
                        <table style="width:100%;border-collapse:separate;border-spacing:2.8mm;margin-top:3mm;">
                            <tr>
                                <td><div class="label">Nickname</div><div class="field" style="font-size:7.4pt;">{{ $student['nickname'] ?: '—' }}</div></td>
                                <td><div class="label">Hometown</div><div class="field" style="font-size:7.4pt;">{{ $student['hometown'] ?: '—' }}</div></td>
                                <td><div class="label">Most Likely To</div><div class="field" style="font-size:7.4pt;">{{ $student['most_likely_to'] ?: '—' }}</div></td>
                            </tr>
                            <tr>
                                <td colspan="3"><div class="label">Achievements</div><div class="field" style="font-size:7.4pt;line-height:1.45;">{{ $limit($student['achievements'], 170) ?: '—' }}</div></td>
                            </tr>
                            <tr>
                                <td colspan="3"><div class="label">Future Plans</div><div class="field" style="font-size:7.4pt;line-height:1.45;">{{ $limit($student['future_plans'] ?: $student['ambition'], 170) ?: '—' }}</div></td>
                            </tr>
                            <tr>
                                <td colspan="3"><div class="label">Messages</div><div class="field" style="font-size:7.4pt;line-height:1.45;">{{ $limit(trim(($student['message_to_batchmates'] ?? '').' '.($student['message_to_parents'] ?? '')), 180) ?: '—' }}</div></td>
                            </tr>
                        </table>
                    </td>
                </tr>
...
<div class="page paper">
    <div style="margin-top:42mm;width:150mm;">
