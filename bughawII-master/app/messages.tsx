...
import { fetchCurrentUser, getConversations, getErrorMessage, getMessageThread, imageUrl, markMessageRead, sendMessage, unwrap } from '../lib/api';
...
export default function MessagesScreen() {
  ...
  const handleSend = async () => {
    ...
    try {
      setSending(true);
      const sent = await sendMessage(personId(selected, currentUser?.id), message.trim());
      setThread((current) => [...current, unwrap(sent)]);
      setMessage('');
      ...
      <Modal visible={!!selected} animationType="slide" onRequestClose={() => setSelected(null)}>
        <SafeAreaView style={styles.container}>
          <View style={styles.threadHeader}>
            ...
            <Text style={styles.threadTitle}>{personName(selected, currentUser?.id)}</Text>
            <View style={{ width: 20 }} />
          </View>
          <FlatList
            data={thread}
            ...
            renderItem={({ item }) => (
              <View style={[styles.bubble, item?.sender_id === currentUser?.id ? styles.mineBubble : null]}>
                ...
                <Text style={[styles.bubbleTime, item?.sender_id === currentUser?.id ? styles.mineTime : null]}>{formatTime(item?.created_at)}</Text>
              </View>
            )}
          ...
          <View style={styles.composer}>
            <TextInput style={styles.composerInput} placeholder="Write a message..." placeholderTextColor="#94a3b8" value={message} onChangeText={setMessage} multiline />
...
const styles = StyleSheet.create({
  ...
  avatarFallback: { width: 52, height: 52, borderRadius: 26, marginRight: 14, backgroundColor: '#1d2b4b', justifyContent: 'center', alignItems: 'center' },
