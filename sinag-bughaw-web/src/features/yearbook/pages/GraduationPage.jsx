...
function UploadModal({ tab, albums, onClose, onSuccess }) {
  ...
  const handleDrop = (e) => {
    ...
    setFiles(cfg.multiple ? dropped : [dropped[0]]);
  };
        ...
        <div className="px-7 py-6 flex flex-col gap-5">
          ...
          <div>
            ...
            <label
              ...
              style={{ border: '2px dashed #e2e8f0', padding: '28px 20px', background: files.length ? 'rgba(63,81,181,0.04)' : '#f8fafc', minHeight: '110px' }}>
              ...
              {files.length
                ? <div className="text-center">
                    ...
                    <p className="text-xs mt-1 m-0" style={{ color: '#94a3b8' }}>{files.map(f => f.name).join(', ').slice(0, 55)}{files.map(f => f.name).join(', ').length > 55 ? '…' : ''}</p>
                  </div>
          ...
          {loading && progress > 0 && (
            <div>
              ...
              <div className="w-full rounded-full" style={{ height: '6px', background: '#e2e8f0' }}>
                <div className="rounded-full transition-all" style={{ width: `${progress}%`, height: '6px', background: '#3f51b5' }} />
              </div>
            </div>
          )}
          ...
          <button onClick={handleSubmit} disabled={loading}
            className="w-full flex items-center justify-center gap-2 font-bold text-white py-3.5 rounded-2xl border-none cursor-pointer transition-all text-sm"
  ...
  return (
    <div className="rounded-3xl overflow-hidden bg-white"
      ...
      <div style={{ padding: '20px 22px' }}>
        ...
        {/* Deep-link to speeches page (premium) */}
  ...
  return (
    <div className="rounded-3xl bg-white overflow-hidden"
      ...
      <div className="relative flex items-center justify-center"
        ...
        {[...Array(20)].map((_,i) => (
          <div key={i} className="absolute" style={{ width: '3px', borderRadius: '3px',
            height: `${20+Math.sin(i*0.8)*36}px`,
            background: playing ? '#fdb813' : 'rgba(253,184,19,0.25)',
            left: `${4+i*4.8}%`, transition: 'background 0.3s' }} />
        ))}
        <button onClick={toggle} className="relative z-10 flex items-center justify-center rounded-full border-none cursor-pointer"
...
export default function GraduationPage() {
  ...
  const isAdmin                      = user?.role === 'admin' || user?.is_admin || user?.is_premium;
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#f8fafc', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <header className="text-white text-center"
        ...
        {/* Speeches CTA — visible to all; SubscriberRoute handles the gate */}
        <Link to="/graduation/speeches"
      ...
      <div className="mx-auto px-5" style={{ maxWidth: '1000px', width: '100%', marginTop: '-55px' }}>
        <div className="bg-white flex gap-2 p-2 rounded-2xl mb-2" style={{ boxShadow: '0 18px 36px rgba(29,43,75,0.1)' }}>
          {primaryTabs.map(tab => (
        ...
        <div className="bg-white flex gap-2 p-2 rounded-2xl" style={{ boxShadow: '0 8px 24px rgba(29,43,75,0.07)' }}>
